...
namespace MMMaellon.BigDeckIsBackInTown
{
    ...
    public class CardThrowing : SmartObjectSyncState
    {
        ...
        {
            if (!allow_throwing || (!throw_primed && first_throw_only))
            {
                return;
            }
            throw_primed = false;
        ...
        public void OnThrow()
        {
            if (!card.deck.throwing_handler && sync.state != SmartObjectSync.STATE_INTERPOLATING)
            {
                return;
            }
            if (desktop_throw_assist && !sync.owner.IsUserInVR())
            {
                if (sync.rigid.velocity == Vector3.zero)
                {
                    sync.rigid.velocity = sync.owner.GetTrackingData(VRCPlayerApi.TrackingDataType.Head).rotation * Vector3.forward;
                }
                sync.rigid.velocity = sync.rigid.velocity.normalized * (2 * throw_min_velocity);
            }
            else if (sync.rigid.velocity.magnitude < throw_min_velocity)
            {
                return;
            }
            temp_target = card.deck.throwing_handler.LocateBestCard(transform.position, card.sync.rigid.velocity, throw_threshold);
            if (temp_target)
            {
